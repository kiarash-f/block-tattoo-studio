...
- ❌ **CODE — Multi-purpose vouchers taxed at the wrong time:** see **H7**. Until fixed, do not sell MULTI_PURPOSE products.
...
### 8.2 Invoicing / §14 UStG — nothing exists
**CODE + BUSINESS/LEGAL.** There is no invoice or receipt generation, **no invoice-number sequence, no invoice model at all** (grep for invoice/Rechnung: zero hits). Kleinbetragsrechnung (≤ €250 gross) may cover many tattoo payments, but voucher sales and guest-table bookings routinely exceed €250, and B2B guest artists will ask for proper invoices. Minimum code work: sequential, gap-free invoice numbering + a generated document per payment with the §14 mandatory fields (studio identity, tax number, date, amount, VAT rate/amount). Whether receipts suffice for which sales: Steuerberater.
...
- ⚠️ **CODE:** `cancelPayment` rewrites `note` in place ([payments.service.ts:230](src/payments/payments.service.ts:230)) — prior note content isn't versioned; put cancellation reasons in their own append-only field/table.
- ⚠️ **CODE:** `paidAt` is arbitrarily backdatable/future-datable via the DTO with no bounds — add sanity bounds (e.g. not in the future, not before go-live) to keep records plausible.
- ⚠️ **CODE:** hard deletes exist for `GuestArtistBooking` ([guest-artist-bookings.service.ts:331](src/guest-artist-bookings/guest-artist-bookings.service.ts:331) — Restrict protects *paid* ones only) and `TattooSession` ([scheduling.service.ts:271](src/scheduling/scheduling.service.ts:271)). If these records feed financial context (they do — payments reference bookings), prefer soft-delete/archive.

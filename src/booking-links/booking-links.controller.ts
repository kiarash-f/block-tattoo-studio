...
export class BookingLinksController {
  ...
  @Post(':id/links')
  @ApiOperation({ summary: 'Create tokenized booking link (admin)', description: 'Generates a short-lived, scoped token URL that allows a client to take specific actions on their booking request without logging in.' })
  @ApiParam({ name: 'id', description: 'BookingRequest ID' })
  @ApiResponse({ status: 201, description: 'Booking link created. Returns token and expiry.' })
